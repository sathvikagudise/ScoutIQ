...
# SameSite=None is the only value shared cross-site (Vercel frontend -> Render
# backend); browsers ignore SameSite=None cookies without the Secure flag, so
# Secure is always forced on when "none" is selected.
VALID_SESSION_COOKIE_SAMESITE = ("lax", "strict", "none")
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        if samesite not in VALID_SESSION_COOKIE_SAMESITE:
            samesite = DEFAULT_SESSION_COOKIE_SAMESITE
        secure = os.getenv("SESSION_COOKIE_SECURE", "false").lower() in (
            "1",
            "true",
            "yes",
        )
